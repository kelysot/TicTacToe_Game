...
public class TicTacToeBoard extends View {
    ...
    //make the board perfect to user screen
    @Override
        ...
        if(action == MotionEvent.ACTION_DOWN){
            ...
            if(!winningLine){
                if(game.updateGameBoard(row, col)){
                    invalidate(); //update graphic func
                    ...
                    if(game.getPlayer() % 2 == 0){ //we know now play player 2 - O
                        game.setPlayer(game.getPlayer() - 1);
                    }
                    else { //we know now play player 1 - X
                        game.setPlayer(game.getPlayer() + 1);
                    }
                }
            }
        ...
        for (int r = 0; r < 3; r++){
            for (int c = 0; c < 3; c++){
                if(game.getGameBoard()[r][c] != 0){
                    if(game.getGameBoard()[r][c] == 1) // 1 is X player
                        drawX(canvas, r, c);
                    else //O is O player
                        drawO(canvas, r, c);
                }
    ...
    public void setUpGame(Button playAgain, Button home, TextView player, String[] names,
                          TextView player1Score, TextView player2Score){
